...
export default function EditJobForm({ job }: EditJobFormProps) {
  ...
  const form = useForm({
    initialValues: {
      ...
      isRemote: job.isRemote === true ? "true" : job.isRemote === false ? "false" : "",
      salaryMin: job.salaryMin?.toString() ?? "",
      salaryMax: job.salaryMax?.toString() ?? "",
      salaryCurrency: job.salaryCurrency ?? "MAD",
    ...
    validationSchema: updateJobOfferSchema as any,
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      return await doEditJob(transformedData);
    },
    onSuccess: (result) => {
      if (result?.data) {
