...
export async function POST(req: NextRequest) {
    try {
        ...
        // Generate unique filename - always output as webp for better compression
        const uniqueId = crypto.randomUUID();
        const outputExt = ext === '.svg' || ext === '.gif' ? ext : '.webp';
        const filename = `${uniqueId}${outputExt}`;

        // Create upload directory if it doesn't exist
        const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'blog', 'articles');
        ...
        const url = `/uploads/blog/articles/${filename}`;
