      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6">
        ...
        {!loading && (
          <div className="space-y-3 sm:space-y-5 mb-8 sm:mb-10">
            ...
            <div
              key={job.id}
              className={`relative bg-white border-2 border-gray-200  sm:rounded-3xl p-3 sm:p-5 hover:border-gray-300 hover:shadow transition-all duration-300 group ${
            ...
            >
              ...
              {job.featured && (
                <div className="absolute -top-2 left-4 sm:-top-3 sm:left-8">
                  <div className="bg-gradient-to-r from-emerald-700 to-emerald-800 text-white px-2 sm:px-3 py-0.5 sm:py-1  text-xs sm:text-sm font-medium flex items-center gap-1 sm:gap-2 shadow">
              ...
              {job.urgent && (
                <div className="absolute -top-2 right-4 sm:-top-3 sm:right-8">
                  <div className="bg-gradient-to-r from-red-500 to-pink-600 text-white px-2 sm:px-3 py-0.5 sm:py-1  text-xs sm:text-sm font-medium shadow">
                    Urgent
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                <div className="flex items-start gap-2 sm:gap-4 flex-1">
                  ...
                  <div className="w-10 h-10 sm:w-14 sm:h-14 bg-gradient-to-br from-emerald-50 to-emerald-50  sm: flex items-center justify-center flex-shrink-0 border-2 border-gray-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-1 sm:gap-2 mb-1 sm:mb-2">
                      ...
                      <div className="flex gap-1 flex-wrap">
                        <span className={`px-2 sm:px-2.5 py-0.5  text-xs sm:text-sm font-medium border-2 ${getTypeColor(job.type)}`}>
                          {getTypeLabel(job.type)}
                        </span>
                        {job.remote && (
                          <span className="px-2 sm:px-2.5 py-0.5 bg-emerald-50 text-emerald-800 border-2 border-emerald-200  text-xs sm:text-sm font-medium">
                            Remote
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1 sm:gap-1.5">
                      {job.requirements.slice(0, 3).map((req, reqIndex) => (
                        <span
                          key={reqIndex}
                          className="px-2 sm:px-2.5 py-0.5 bg-gray-50 text-gray-700 text-xs sm:text-sm rounded-lg sm: border border-gray-200"
                        >
                          {req}
                        </span>
                      ))}
                      {job.requirements.length > 3 && (
                        <span className="px-2 sm:px-2.5 py-0.5 bg-gray-100 text-gray-500 text-xs sm:text-sm rounded-lg sm: border border-gray-200">
                          +{job.requirements.length - 3}
                        </span>
                      )}
                    </div>
                ...
                <div className="flex flex-col gap-1 sm:gap-2 sm:items-end mt-2 sm:mt-0">
                  <Link
                    ...
                    className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2.5 bg-gradient-to-r from-emerald-700 to-emerald-800 text-white  sm: font-semibold hover:shadow hover:shadow-emerald-200 transition-all duration-300 flex items-center justify-center gap-2 sm:gap-2 group text-xs sm:text-sm"
                  >
